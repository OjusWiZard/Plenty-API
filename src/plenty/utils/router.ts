...
import { IConfigToken, TokenStandard } from "../plenty.types";
...
export const routerSwap = async (
	...
	const tokenIn = plenty.getTokenBySymbol(path[0]) as IConfigToken;
	const routerInstance = await tezos.provider.wallet.at(plenty.router);

	let DataLiteral: any = [];
	for (let i = 0; i < path.length - 1; i++) {
		const dex = await plenty.poolFromPair(path[i], path[i + 1], tezos);
		const pathI1 = plenty.getTokenBySymbol(path[i + 1]) as IConfigToken;
	...
	if (tokenInCallType === TokenStandard.TEZ) {
		...
		const tokenInInstance: any = await tezos.provider.wallet.at(tokenIn.address as string);
	...
	return await getBatchOperationsWithLimits(tezos.provider, allBatchOperations);
};
...
export const getBatchOperationsWithLimits = async (
	...
	try {
		...
		throw new Error('Plenty: ' + error.message);
	}
};
